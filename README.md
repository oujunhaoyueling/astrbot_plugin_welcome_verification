# 重要！！此插件与 'meme_manager' 冲突，同时安装会导致此插件功能不可用（恢复会通知）建议使用 '表情包小偷' 等效替代 'meme_manager' 
# AstrBot 入群欢迎与验证插件
...
| 命令 | 说明 | 权限 |
|------|------|------|
| `welcome` | 查看当前群组欢迎配置和命令列表 | 所有人 |
| `welcome text <内容>` | 设置群组欢迎文本（支持 {user_name} 变量） | 管理员/群主 |
| `welcome image <路径/URL>` | 设置群组欢迎图片 | 管理员/群主 |
| `welcome image on` | 启用群组欢迎图片 | 管理员/群主 |
| `welcome image off` | 禁用群组欢迎图片 | 管理员/群主 |
| `welcome reset` | 重置为全局配置 | 管理员/群主 |

**使用示例**：
```
welcome text 欢迎 {user_name} 来到我们的技术交流群！
welcome image https://example.com/group-banner.jpg
welcome reset
```
