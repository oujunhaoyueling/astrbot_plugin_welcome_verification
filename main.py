...
class WelcomeVerificationPlugin(Star):
    ...
    async def _check_answer(self, event: AstrMessageEvent):
        ...
        answer_num = self._extract_number_from_text(plain_text)
        ...
        async with self._lock:
            ...
            if answer_num is None:
                ...
                if future and not future.done():
                    future.set_result(answer_num == correct)
                return

        if answer_num is None:
            await event.send(event.plain_result("请输入数字答案（回复内容中包含数字即可）。"))
    ...
    async def terminate(self):
        async with self._lock:
            ...
            for task in self.timeout_kick_tasks.values():
                if not task.done():
                    task.cancel()
            self.timeout_kick_tasks.clear()        self._load_group_configs()
        self._load_all_question_banks()
