...
@register("astrbot_plugin_welcome_verification", "YourName", "入群欢迎与验证插件", "2.3.0")
class WelcomeVerificationPlugin(Star):
    ...
    def _normalize_text(self, text: str) -> str:
        """标准化文本用于比较：去除非字母数字、转小写、去除首尾空格"""
        # 保留中文字符、字母、数字，去除其他标点符号
        normalized = re.sub(r'[^\w\u4e00-\u9fff]', '', text.lower())
        return normalized.strip()

    def _get_plain_text(self, event: AstrMessageEvent) -> str:
        """从消息中提取纯文本内容（排除 At 等组件）"""
        text_parts = []
    ...
    def _is_answer_match(self, user_input: str, correct_answer: Union[str, int, float]) -> bool:
        ...
        if isinstance(correct_answer, (int, float)):
            ...
            if isinstance(correct_answer, float):
                return abs(user_num - correct_answer) < 1e-6
    ...
    async def _handle_wv_command(self, event: AstrMessageEvent):
        ...
        if len(parts) < 2:
            help_text = (
                "入群验证管理命令：\n"
                "wv on - 开启本群入群验证（仅管理员/群主）\n"
                "wv off - 关闭本群入群验证（仅管理员/群主）\n"
                "wv ls - 查看可用题库\n"
                "wv <文件名> - 切换题库\n"
                "wv default - 恢复随机生成"
            )
        ...
        if subcmd == "on":
            if not is_admin:
                await event.send(event.plain_result("只有管理员或群主可以开启验证。"))
                return
            self._set_group_verification_enabled(group_id, True)
            await event.send(event.plain_result("已开启本群的入群验证。"))
            return

        if subcmd == "off":
            if not is_admin:
                await event.send(event.plain_result("只有管理员或群主可以关闭验证。"))
                return
            self._set_group_verification_enabled(group_id, False)
            await event.send(event.plain_result("已关闭本群的入群验证。"))
            return
        ...
        if subcmd == "ls":
            ...
            if banks:
                ...
                msg = "没有发现任何题库文件，请将 JSON 格式的题库放入 data/plugin_data/welcome_verification/warehouse/ 文件夹。"
    ...
    async def _check_answer(self, event: AstrMessageEvent):
        ...
        async with self._lock:
            ...
            if is_match:
                ...
                # 答案错误，提示（但不在这里设置 False，因为还有机会重试）
                # 注意：这里不直接 set_result(False)，因为 future 需要等待下一次消息
                # 而是在外部处理错误计数，这里只是判断并返回结果
                # 实际上，主流程中 future 的结果为 True 表示正确，超时或手动计数错误
                # 为了与原有逻辑兼容，我们在这里只处理正确的情况，错误的情况让外部超时或次数用完处理
                # 但为了即时反馈，可以发送错误提示，但不会立即结束 future
                # 修改：直接设置结果为 False，让外部循环知道这次错误
                future.set_result(False)
                # 发送错误提示（锁外发送）
                async def send_wrong():
                    # 获取剩余次数等信息需要锁外，这里简化：直接提示错误
                    await event.send(event.plain_result("答案错误。"))
                asyncio.create_task(send_wrong())
